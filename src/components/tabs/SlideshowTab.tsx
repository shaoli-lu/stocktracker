        ...
        <AnimatePresence mode="wait">
          ...
          >
            {profile && profile.logo ? (
              <img src={profile.logo} alt={sy} className="w-24 h-24 mb-6 rounded-2xl shadow-2xl bg-white/10 object-contain p-2 ring-1 ring-white/20" />
            ) : (
               <div className="w-24 h-24 mb-6 rounded-2xl bg-gradient-to-br from-gray-800 to-gray-700 flex items-center justify-center shadow-2xl ring-1 ring-white/20">
                 <DollarSign className="w-12 h-12 text-gray-400" />
               </div>
            )}
            
            <h2 className="text-5xl font-black mb-2 tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">
              {profile ? profile.name : sy}
            </h2>
            <div className="text-xl text-indigo-400 font-bold tracking-widest uppercase mb-8 opacity-80">{sy}</div>
            
            {quote && (
              <div className="flex flex-col items-center">
                <span className="text-6xl font-black text-white mb-4 tracking-tighter">
                  ${quote.c?.toFixed(2) || "0.00"}
                </span>
                
                <div className={`flex items-center gap-3 px-6 py-2.5 rounded-full font-bold text-xl ${isUp ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
